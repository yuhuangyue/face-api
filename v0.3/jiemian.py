...
class MyThread1(threading.Thread):
    ...
    def run(self):
        global age_res
        global gender_res
        while((tasklist[0]==0).all()):
            time.sleep(1)
        step = 0
        # print(tasklist[step])
        while((tasklist[step]!=-1).all()):
            if (tasklist[step]==0).all():
                continue
            demography = DeepFace.analyze(tasklist[step], actions = ['age', 'gender'],enforce_detection = False,models=models,detector_backend = backends[0])
